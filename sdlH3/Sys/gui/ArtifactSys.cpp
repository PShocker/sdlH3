...
static void receive() {
  ...
  World::iterateSystems.push_back([afComp]() {
    ...
    HeroScrSys::heroArtifactEquip(heroComp,
                                  heroComp.artifactsInBackpack.size() - 1);
    return true;
  });
...
static void drawButton() {
  ...
  AdvMapSys::drawButtons(leftUp.x, leftUp.y, top, ArtifactSys::buttons);
}
...
bool ArtifactSys::leftMouseUp(float x, float y) {
  ...
  if (AdvMapSys::clickButtons(leftUp.x, leftUp.y, ArtifactSys::buttons,
                              clickType)) {
    return false;
  }
  return true;
