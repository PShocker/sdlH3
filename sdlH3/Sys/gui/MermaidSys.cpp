...
static void drawBackGround() {
  ...
  auto oName = strPool[927 + ObjectType::MERMAID];
...
static void drawButton() {
  ...
  auto top = (*topFunc.target<bool (*)()>() == MermaidSys::run);
  AdvMapSys::drawButtons(leftUp.x, leftUp.y, top, MermaidSys::buttons);
}
...
bool MermaidSys::leftMouseUp(float x, float y) {
  SDL_FPoint leftUp{Global::viewPort.w / 2 - bakW / 2,
                    Global::viewPort.h / 2 - bakH / 2};
  auto clickType = (uint8_t)Enum::CLICKTYPE::L_UP;

  if (AdvMapSys::clickButtons(leftUp.x, leftUp.y, MermaidSys::buttons,
                              clickType)) {
    return false;
  }
  return true;
...
bool MermaidSys::keyUp(uint16_t key) {
  switch (key) {
