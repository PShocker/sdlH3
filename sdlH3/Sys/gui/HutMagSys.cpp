  ...
  World::iterateSystems.push_back([]() {
    if (Global::hutMagIndex >= 0) {
      ...
      for (const auto point : FogSys::generateVerticalHexagon(5)) {
        ...
        Global::fogs[playerIdComp->id][World::level]
                    [rX + rY * Global::mapSize] = true;
      }
...
static void drawButton() {
  ...
  AdvMapSys::drawButtons(leftUp.x, leftUp.y, top, HutMagSys::buttons);
}
...
bool HutMagSys::leftMouseUp(float x, float y) {
  ...
  if (AdvMapSys::clickButtons(leftUp.x, leftUp.y, HutMagSys::buttons,
                              clickType)) {
    return false;
  }
  return true;
