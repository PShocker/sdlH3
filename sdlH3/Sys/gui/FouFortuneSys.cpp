...
static bool visited() {
  ...
  auto &fComp =
      World::registrys[World::level].get<FouFortuneComp>(Global::goalEnt);
  if (fComp.visitHeros.contains(heroComp.portrait)) {
    return true;
  } else {
    return false;
  }
...
static void drawBackGround() {
  ...
  auto oName = strPool[927 + ObjectType::FOUNTAIN_OF_YOUTH];
  FreeTypeSys::drawCenter(x + bakW / 2, y - bakH / 2 + 50, oName);
...
bool FouFortuneSys::leftMouseUp(float x, float y) {
  SDL_FPoint leftUp{Global::viewPort.w / 2 - bakW / 2,
                    Global::viewPort.h / 2 - bakH / 2};
...
bool FouFortuneSys::rightMouseDown(float x, float y) {
...
bool FouFortuneSys::keyUp(uint16_t key) {
  switch (key) {
