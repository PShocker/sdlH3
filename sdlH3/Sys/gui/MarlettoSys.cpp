...
static void drawBackGround() {
  ...
  FreeTypeSys::drawCenter(Global::viewPort.w / 2, y - bakH / 2,
                          strPool[926 + (uint8_t)ObjectType::MARLETTO_TOWER]);
  return;
}
...
bool MarlettoSys::leftMouseUp(float x, float y) {
  SDL_FPoint leftUp{Global::viewPort.w / 2 - bakW / 2,
                    Global::viewPort.h / 2 - bakH / 2};
  auto v = buttonInfo();
...
bool MarlettoSys::rightMouseDown(float x, float y) {
...
bool MarlettoSys::keyUp(uint16_t key) {
  switch (key) {
