...
static void receive() {
  ...
  if (!heroComp.visited.contains((uint8_t)ObjectType::MAGIC_WELL)) {
  }
  heroComp.visited.insert((uint8_t)ObjectType::MAGIC_WELL);
...
static void drawBackGround() {
  ...
  FreeTypeSys::drawCenter(Global::viewPort.w / 2, y - bakH / 2 + 15,
                          strPool[926 + (uint8_t)ObjectType::MAGIC_WELL]);
...
bool MagWellSys::leftMouseUp(float x, float y) {
  SDL_FPoint leftUp{Global::viewPort.w / 2 - bakW / 2,
                    Global::viewPort.h / 2 - bakH / 2};
  auto v = buttonInfo();
...
bool MagWellSys::rightMouseDown(float x, float y) {
...
bool MagWellSys::keyUp(uint16_t key) {
  switch (key) {
